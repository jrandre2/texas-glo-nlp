#!/usr/bin/env python3
"""
Create a census tract choropleth for all mentions (no time filter).
"""
...
def build_tract_map(df: pd.DataFrame, geojson: dict, prop_key: str, title: str, output_html: Path):
    ...
    # Use a stronger colorscale and log transform for better contrast on low counts
...
def main():
    ...
    build_tract_map(df, geojson, prop_key, "Tract Mentions - All Data", Path(args.output))
